...
`x0x.md` is intended to retain and serve verified installer and artefact bytes independently during GitHub outages. Every alternate delivery must preserve the canonical source and release identity and the associated hashes, signatures, manifests, and provenance. A replica, mirror, cache, retained store, or replacement backend is delivery infrastructure only and must never become an alternate product or release authority.
...
- With GitHub delivery unavailable and transient caches cleared, install and retrieve independently retained bytes through `x0x.md`; verify their canonical source/release identity, hashes, signatures, manifests, and provenance against the authoritative set.
- Compare bytes and verification metadata obtained through multiple advertised delivery choices; require equality for the selected canonical source or release and reject stale, corrupt, partial, unsigned, or cross-release combinations.
